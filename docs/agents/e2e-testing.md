...
- **The lane sets `migrations.etcdAdoptSkipBackup: true`** in the baseline platform Package. An etcd-adoption safety snapshot needs a trusted-cert external S3 endpoint the sandbox can't provide (`example.org`, no DNS/ACME, self-signed in-cluster cert), so it is skipped while the adoption itself still runs. `etcdAdoptSkipBackup` is a first-class, default-off platform value — real upgrades still snapshot.
